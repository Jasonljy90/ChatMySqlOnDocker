...
func insertRecord(userName, passWord, firstName, lastName, language string) {
	_, err := db.Exec("INSERT INTO userAccountDB.Users VALUES (?,?,?,?,?)", userName, passWord, firstName, lastName, language)
	if err != nil {
...
func deleteRecord(userName string) int {
	results, err := db.Exec("DELETE FROM userAccountDB.Users where Username=?", userName)
	if err != nil {
...
func changePasswordRecord(userName string, password []byte) {
	results, err := db.Query("SELECT * FROM userAccountDB.Users where Username=?", userName)
	if err != nil {
		fmt.Println(err)
	} else {
		for results.Next() {
			...
			if err != nil {
				...
				_, err := db.Exec("UPDATE userAccountDB.Users set password = ?, firstname = ?, lastname = ?, language = ? where Username=?", password, &person.FirstName, &person.LastName, &person.Language, &person.UserName)
				if err != nil {
					panic(err)
				}
			}
	...
	if err != nil {
		...
		for results.Next() {
			...
			if err != nil {
				...
				_, err := db.Exec("UPDATE userAccountDB.Users set language = ?, firstname = ?, lastname = ? where Username=?", language, &person.FirstName, &person.LastName, &person.UserName)
				if err != nil {
					panic(err)
				}
			}
